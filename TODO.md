...
[ ] this issue with /data/music and double the number of tracks in the database has happened MUTLIPLE times. Beyond fixing it, there must be a way to add an automated test to prevent this. We tried to prevent it with the hash matching, but that apparently didn't fully work.