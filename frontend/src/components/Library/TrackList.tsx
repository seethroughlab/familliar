...
function TrackRow({ track, index, isCurrentTrack, isPlaying, onPlay }: TrackRowProps) {
  return (
    <div
      className={`group grid grid-cols-[3rem_1fr_1fr_1fr_5rem] gap-4 px-4 py-2 rounded-md hover:bg-zinc-800/50 ${
...
export function TrackList({ search, artist, album }: TrackListProps) {
  ...
  return (
    <div>
      {/* Header */}
      <div className="grid grid-cols-[3rem_1fr_1fr_1fr_5rem] gap-4 px-4 py-2 text-sm text-zinc-400 border-b border-zinc-800">
        <div>#</div>
        <div>Title</div>
        <div>Artist</div>
        <div>Album</div>
